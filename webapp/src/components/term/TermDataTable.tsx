...
export default function TermDataTable({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedTerm?.id) {
        ...
        toast.success("Cập nhật kế hoạch NCKH thành công");
      } else {
        await createTerm(formData as CreateTermDto);
        toast.success("Thêm kế hoạch NCKH thành công");
      }
      closeModal();
      onRefresh();
    } catch (error: unknown) {
      console.error('Error in handleSubmit:', error);
      const errorMessage = selectedTerm?.id ? "Không thể cập nhật kế hoạch NCKH" : "Không thể thêm kế hoạch NCKH";
  ...
  const handleDelete = async (id: number) => {
    ...
    const isConfirmed = window.confirm("Bạn có chắc chắn muốn xóa kế hoạch NCKH này?");
    if (!isConfirmed) return;

    try {
      setIsSubmitting(true);
      await deleteTerm(id.toString());
      toast.success("Xóa kế hoạch NCKH thành công");
      onRefresh();
    } catch (error: unknown) {
      console.error('Error in handleDelete:', error);
      
      let errorMessage = "Không thể xóa kế hoạch NCKH";
      
      if (error instanceof Error && error.message) {
        if (error.message.includes("Không thể xóa kế hoạch NCKH này vì đang được sử dụng trong:")) {
  ...
  const renderRow = (term: Term) => (
    <TableRow key={term.id}>
      ...
      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
        <div className="text-sm">
          <div>Từ: {formatDate(term.startDate)}</div>
          <div>Đến: {formatDate(term.endDate)}</div>
        </div>
      </TableCell>
      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
        <Badge
          size="sm"
          color={term.status === 'open' ? "success" : term.status === 'closed' ? "warning" : "light"}
        >
          {term.status === 'open' ? "Đang mở" : term.status === 'closed' ? "Đã đóng" : "Đã lưu trữ"}
        </Badge>
      </TableCell>
      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
        <div className="space-y-2">
          <div className="text-sm font-medium">Milestones ({term.termMilestones?.length || 0})</div>
          {term.termMilestones && term.termMilestones.length > 0 && (
                ...
                .map((milestone) => (
                  <div key={milestone.id} className="flex items-center justify-between text-xs bg-gray-50 dark:bg-gray-800 p-2 rounded">
                    ...
                    <div className="flex gap-1">
                      <button
                        onClick={() => handleEditMilestone(term, milestone)}
      ...
      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
        <div className="flex items-center gap-3">
          <button
            title="Thêm cột mốc"
            onClick={() => handleAddMilestone(term)}
  ...
  return (
    <>
      <SearchableDataTable
        ...
        onSearch={onSearch}
        searchPlaceholder="Tìm kiếm theo tên kế hoạch NCKH..."
        ...
        <div className="flex flex-col px-2 overflow-y-auto custom-scrollbar">
          <div>
            <h5 className="mb-2 font-semibold text-gray-800 modal-title text-theme-xl dark:text-white/90 lg:text-2xl">
              {selectedTerm ? "Chỉnh sửa kế hoạch NCKH" : "Thêm kế hoạch NCKH"}
            </h5>
          </div>
          <div className="mt-8">
            <div className="grid grid-cols-2 gap-4">
              <div className="mb-3">
                ...
                <input
                  ...
                  placeholder="Nhập tên kế hoạch NCKH"
                />
              </div>
              <div className="mb-3">
                <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                ...
                <input
                  ...
                  className="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800"
                  placeholder="Nhập mã kế hoạch NCKH"
                />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
            ...
            <div className="mb-3">
              ...
              <textarea
                ...
                placeholder="Nhập mô tả kế hoạch NCKH (không bắt buộc)"
              />
            </div>
            <div className="mb-3">
              <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
            ...
            <div className="mb-3">
              ...
              <select
                id="status"
                value={formData?.status as TermStatus}
          ...
          <div>
            ...
            {selectedTerm && (
              <p className="text-base text-gray-600 dark:text-gray-400">
                Kế hoạch NCKH: {selectedTerm.name}
              </p>
            )}
          </div>
          <div className="mt-8">
            ...
            <div className="mb-3">
              ...
              <select
                id="status"
                value={milestoneFormData?.status as TermMilestoneStatus}
