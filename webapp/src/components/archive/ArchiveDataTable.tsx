...
export default function ArchiveDataTable({ 
  ...
  const renderRow = (item: ProjectEntity) => (
    <TableRow key={item.id}>
      ...
      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-base dark:text-gray-400">
        {item.lastMilestoneSubmission ? item.lastMilestoneSubmission.fileUrl : '-'}
        {/* {item.lastMilestoneSubmission?.submittedAt ? new Date(item.lastMilestoneSubmission.submittedAt).toLocaleString('vi-VN') : '-'} */}
      </TableCell>
      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
